...
for ($i=0; $i<10 && !$done; $i++) {
	...
	print "Date: ";
	system("date");
	print "Command: runat $ec2timeout ec2-describe-instances\n";
	print "\n";
	system("runat $ec2timeout ec2-describe-instances");
	print "\n";
	...
	print "Date: ";
	system("date");
	print "Command: runat $ec2timeout ec2-describe-instances | grep INST | awk '{print \$2}'\n";
	print "\n";
	chomp($instIds=`runat $ec2timeout ec2-describe-instances | grep INST | awk '{print \$2}'`);
	$instIds=~s/\n/ /g;
	...
	if ($instIds) {
		...
		foreach my $myid (@temp_array){
			if( request_terminate_instance("ec2", $ec2timeout, $myid ) ){
				$done++;
			};
			sleep(1);
		};
